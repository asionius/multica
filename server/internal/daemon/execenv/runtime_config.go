...
// For CodeBuddy: writes {workDir}/CODEBUDDY.md (skills discovered natively from .codebuddy/skills/)
func InjectRuntimeConfig(workDir, provider string, ctx TaskContextForEnv) (string, error) {
	...
	case "codebuddy":
		return content, os.WriteFile(filepath.Join(workDir, "CODEBUDDY.md"), []byte(content), 0o644)
	default:
...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	b.WriteString("- `multica issue create --title \"...\" [--description \"...\" | --description-stdin | --description-file <path>] [--priority X] [--status X] [--assignee X | --assignee-id <uuid>] [--parent <issue-id>] [--project <project-id>] [--runtime <runtime-id>] [--due-date <RFC3339>] [--attachment <path>]` — Create a new issue; `--attachment` may be repeated. `--runtime` pins the issue to a specific agent_runtime so it dispatches with that runtime's credentials (overrides the agent's default runtime).\n")
	b.WriteString("- `multica issue update <id> [--title X] [--description X | --description-stdin | --description-file <path>] [--priority X] [--status X] [--assignee X | --assignee-id <uuid>] [--parent <issue-id>] [--project <project-id>] [--runtime <runtime-id>] [--due-date <RFC3339>]` — Update issue fields; use `--parent \"\"` to clear parent, `--runtime \"\"` to clear the runtime pin (fall back to agent default).\n")
	...
	if len(ctx.AgentSkills) > 0 {
		...
		case "codex", "copilot", "opencode", "openclaw", "pi", "cursor", "kimi", "kiro", "codebuddy":
			// Codex, Copilot, OpenCode, OpenClaw, Pi, Cursor, Kimi, Kiro, and CodeBuddy discover
			// skills natively from their respective paths. For OpenClaw, the daemon also writes a
