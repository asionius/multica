...
#   curl -fsSL https://raw.githubusercontent.com/asionius/multica/main/scripts/install.sh | bash -s -- --with-server
#
...
# Configuration
# ---------------------------------------------------------------------------
REPO_URL="https://github.com/asionius/multica.git"
REPO_WEB_URL="https://github.com/asionius/multica"  # without .git, for GitHub web APIs
INSTALL_DIR="${MULTICA_INSTALL_DIR:-$HOME/.multica/server}"
BREW_PACKAGE="asionius/tap/multica"
...
detect_os() {
  ...
  irm https://raw.githubusercontent.com/asionius/multica/main/scripts/install.ps1 | iex" ;;
...
install_cli_brew() {
  ...
  if ! brew tap asionius/tap 2>/dev/null; then
...
install_cli_binary() {
  ...
  local url="https://github.com/asionius/multica/releases/download/${latest}/multica-cli-${version}-${OS}-${ARCH}.tar.gz"
  local tmp_dir
  tmp_dir=$(mktemp -d)
...
run_default() {
  ...
  printf "     curl -fsSL https://raw.githubusercontent.com/asionius/multica/main/scripts/install.sh | bash -s -- --with-server\n"
  printf "\n"
}
...
run_with_server() {
  ...
  printf "     curl -fsSL https://raw.githubusercontent.com/asionius/multica/main/scripts/install.sh | bash -s -- --stop\n"
  printf "\n"
}
